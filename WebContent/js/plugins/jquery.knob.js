...
$(function () {
    ...
    var Dial = function (c, opt) {
        ...
        this.stepDraw = function(nv,tv){
            var a = this.angle(tv)                      // Angle
        ...
        this.animate_to = function(){
            var step = opt.max / 100;
            var vs = dojo.window.getBox();
            if(opt.tv < opt.nv){
            	if((opt.tv + step)>=opt.nv){
            		opt.tv = opt.nv;
            		clearInterval(opt.animation_loop);
            ...
            var a = (opt.tv - opt.min) * PI2 / (opt.max - opt.min)                     // Angle
            ,sa = 1.5 * Math.PI + opt.angleOffset   // Previous start angle
            ,sat = sa                               // Start angle
            ,ea = sa + (opt.v - opt.min) * PI2 / (opt.max - opt.min)                // Previous end angle
            ,eat = sat + a                          // End angle
            ,r = opt.width / 2                      // Radius
            ,lw = r * opt.thickness                 // Line width
            ,cgcolor = Dial.getCgColor(opt.cgColor)
            ...
            ctx.beginPath();
            ctx.strokeStyle = opt.fgColor;
            ctx.arc(r, r, r - lw / 2 - 1, sat, eat, false);
            ctx.stroke();
            
            ctx.fillStyle = opt.fgColor;
            ctx.font = "bold "+25*vs.w/1897+"px Arial";
            var text = opt.tv.toFixed(1);
            var textWidth = ctx.measureText(text).width;
            
            ctx.fillText(text, opt.width/2 - textWidth/2 + 1*vs.w/1897, opt.width/2 + 10*vs.w/1897);
        };
        ...
        this.draw = function (nv) {
            ...
            switch (opt.skin) {
                ...
                case 'default' :
                    ...
                    opt.tv = v;
                    opt.nv = nv;
                    opt.animation_loop = setInterval(this.animate_to, 1000/20);
                    ctx.beginPath();
                    ctx.strokeStyle = opt.fgColor;
                    ctx.arc(r, r, r - lw / 2, sat, eat, false);
                    ctx.stroke();
